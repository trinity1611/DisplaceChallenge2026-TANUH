...
- To run **Track 1 – Speaker Diarization (SD)**, update the `wav` field in the `run.sh` script of the `Track1_SD` system to point to the audio files used for `Track4_DS`, and set the `rttm` field to `None`.
- **Run Speaker Diarization baseline** by following the instructions in the README of Track1_SD. 
- To run **Track2 - ASR**, update the `input_audio_folder` field in config file of the `Track2_ASR` with the path of the Track4_DS audio files and set the `input_gt` field as `None`.
